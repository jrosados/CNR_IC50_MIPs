...
```
We started with 4109 'mutation names'. If we exclude mutations with <1% frequency we would end up with 1201 mutations. We are not exluding anything just for the purpose of estimating prevalence.
...
At this point we have 4109'mutation name'. This is the reference database for prevalence and other analysis
```{r, echo=FALSE, message=FALSE}
...
filter(country=="Ivory Coast" | country== "Cameroon" | country== "Mali" |
  ...
  mutate(country = factor(country, 
                           ...
                           labels = c("Côte d'Ivoire","Cameroon","Mali", "Guinea","Republic of the Congo", "Senegal", "Central African Republic", "Chad","Benin", "Gabon"))) %>%
...
filter(country=="Ivory Coast" | country== "Cameroon" | country== "Mali" |
  ...
  mutate(country = factor(country, 
                           ...
                           labels = c("Côte d'Ivoire (128)","Cameroon (104)","Mali (46)", "Guinea (36)","Republic of the Congo (23)", "Senegal (19)",  "Chad (18)","Central African Republic (14)","Gabon (13)","Benin (12)" ))) %>%
group_by(country, haplotype) %>%
  summarise(n = n()) %>%
  ungroup() %>%
  mutate(freq = n/sum(n)) %>%
...
haplotype_CRT_MDR1_trend_time<- CRT_MDR1_haplotypes_associa %>%
   rowwise() %>%
  mutate(`crt-Lys76Thr` = ifelse(`crt-Lys76Thr`==0, "Pfcrt-K76", "Pfcrt-76T"),
         `mdr1-Asn86Tyr` = ifelse(`mdr1-Asn86Tyr`==0,"Pfmdr1-N86","Pfmdr1-86Y")
         ) %>%
  rowwise() %>%
  unite("haplotype",10:ncol(.),sep =" | ", remove = F) %>%
  ...
  summarise(n = n()) %>%
  # ungroup() %>%
  mutate( prevalence = num((n/sum(n)),digits = 2) ,
         m = (qbinom(0.5, size=sum(n), prob=prevalence))/sum(n),
         lim_inf = (qbinom( 0.025, size=sum(n), prob=prevalence))/sum(n),
         lim_sup = (qbinom( 0.975, size=sum(n), prob=prevalence))/sum(n)) %>%
  unique() %>%
  ggplot(aes(year, prevalence, group = haplotype, colour = haplotype)) +
...
# Grouped prevalence of CRT_MDR1 haplotypes before and after 2019
```{r,echo=FALSE, message=FALSE, warning=FALSE}
haplotype_CRT_MDR1_before2019_after2019<- CRT_MDR1_haplotypes_associa %>%
   rowwise() %>%
  mutate(`crt-Lys76Thr` = ifelse(`crt-Lys76Thr`==0, "Pfcrt-K76", "Pfcrt-76T"),
         `mdr1-Asn86Tyr` = ifelse(`mdr1-Asn86Tyr`==0,"Pfmdr1-N86","Pfmdr1-86Y")
         ) %>%
  ...
  mutate(haplotype = factor(haplotype,
                           levels=c("Pfcrt-76T | Pfmdr1-86Y","Pfcrt-76T | Pfmdr1-N86","Pfcrt-K76 | Pfmdr1-86Y", "Pfcrt-K76 | Pfmdr1-N86"),
                           labels = c("crt 76T + mdr186Y","crt 76T + mdr1 N86","crt K76 + mdr1 86Y","crt K76 + mdr1 N86"))) %>%
 ggplot(aes(year, prevalence, colour = haplotype, fill = haplotype)) +
  geom_bar( stat = "identity", color = "black") +
  ylab("Prevalence (%)") + xlab("Year") + theme_classic() +
...
print(haplotype_CRT_MDR1_before2019_after2019)
ggsave("figures/CRT_MDR1_haplotypes_grouped.pdf", units="cm", width=13, height=9, dpi=300)

```
## Stats of prevalence CRT_MDR1 haplotypes before and after 2019
```{r,echo=FALSE, message=FALSE, warning=FALSE}
stats_hap_CRT_MDR1_before2019_after2019<- CRT_MDR1_haplotypes_associa %>%
   rowwise() %>%
  mutate(`crt-Lys76Thr` = ifelse(`crt-Lys76Thr`==0, "Pfcrt-K76", "Pfcrt-76T"),
         `mdr1-Asn86Tyr` = ifelse(`mdr1-Asn86Tyr`==0,"Pfmdr1-N86","Pfmdr1-86Y")
         ) %>%
...
table(stats_hap_CRT_MDR1_before2019_after2019$haplotype,stats_hap_CRT_MDR1_before2019_after2019$year)

chisq.test(stats_hap_CRT_MDR1_before2019_after2019$haplotype,
           stats_hap_CRT_MDR1_before2019_after2019$year)$expected

fisher.test(stats_hap_CRT_MDR1_before2019_after2019$haplotype,
           stats_hap_CRT_MDR1_before2019_after2019$year)

fisher.test(stats_hap_CRT_MDR1_before2019_after2019$haplotype=="Pfcrt-K76 | Pfmdr1-N86",
           stats_hap_CRT_MDR1_before2019_after2019$year)

fisher.test(stats_hap_CRT_MDR1_before2019_after2019$haplotype=="Pfcrt-K76 | Pfmdr1-86Y",
           stats_hap_CRT_MDR1_before2019_after2019$year)

fisher.test(stats_hap_CRT_MDR1_before2019_after2019$haplotype=="Pfcrt-76T | Pfmdr1-N86",
           stats_hap_CRT_MDR1_before2019_after2019$year)

fisher.test(stats_hap_CRT_MDR1_before2019_after2019$haplotype=="Pfcrt-76T | Pfmdr1-86Y",
           stats_hap_CRT_MDR1_before2019_after2019$year)
```
