...
# Grouped frequency of CRT_MDR1 haplotypes before and after 2019
```{r,echo=FALSE, message=FALSE, warning=FALSE}
haplotype_CRT_MDR1_before2019_after2019<- CRT_MDR1_haplotypes_associa %>%
  ...
  mutate(haplotype = factor(haplotype,
                           ...
                           labels = c("crt 76Thr - mdr186Tyr","crt 76Thr - mdr1 Asn86","crt Lys76 - mdr1 86Tyr","crt Lys76 - mdr1 Asn86"))) %>%
 ggplot(aes(year, frequency, colour = haplotype, fill = haplotype)) +
  geom_bar( stat = "identity", color = "black") +
  ylab("Frequency (%)") + xlab("Year") + theme_classic() +
...
```
## Stats of frequency CRT_MDR1 haplotypes before and after 2019
```{r,echo=FALSE, message=FALSE, warning=FALSE}
...
#remove lines acking genotype data for CRT and MDR1
...
#regression model
model_LMF <- lm(as.formula(paste("Lumefantrine", "~ year + combine")), data = data)
summary(model_LMF)
...
#regression model
model_MDAQ <- lm(as.formula(paste("MDAQ", "~ year + combine")), data = data)
summary(model_MDAQ)
